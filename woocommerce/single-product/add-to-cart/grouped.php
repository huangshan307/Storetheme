...
<form class="cart" action="<?php echo esc_url( get_permalink() ); ?>" method="post" enctype='multipart/form-data'>
	<table cellspacing="0" class="woocommerce-grouped-product-list table">
		<tbody>
			<?php
			...
			foreach ( $grouped_products as $grouped_product ) {
				...
				foreach ( $grouped_product_columns as $column_id ) {
					...
					switch ( $column_id ) {
						case 'thumbnail':
							$value = storetheme_get_product_thumbnail_url('shop_catalog', 0, 0, 50, 50);
							break;
						case 'label':
							$value  = '<div id="product-' . esc_attr( $grouped_product->get_id() ) . '">';
							$value .= $grouped_product->is_visible() ? '<a class="text-dark font-weight-bold" href="' . esc_url( apply_filters( 'woocommerce_grouped_product_list_link', get_permalink( $grouped_product->get_id() ), $grouped_product->get_id() ) ) . '">' . $grouped_product->get_name() . '</a>' : $grouped_product->get_name();
							$value .= '</div>';
					...
					echo '<td class="woocommerce-grouped-product-list-item__' . esc_attr( $column_id ) . ' border-0">' . apply_filters( 'woocommerce_grouped_product_list_column_' . $column_id, $value, $grouped_product ) . '</td>'; // WPCS: XSS ok.
	...
	<?php if ( $quantites_required ) : ?>
		...
		<button type="submit" class="single_add_to_cart_button alt btn text-light btn-add-to-cart">
			<i class="fas fa-cart-plus mr-3"></i> <?php echo esc_html( $product->single_add_to_cart_text() ); ?>
		</button>
